...
This plan outlines the integration of four key algorithmic concepts—Divide and Conquer, Dynamic Programming, Sorting, and Recursive Search—to enhance the Nets game engine (`cpp/` module). The primary goal is to build a robust "Exact Solver" and optimize the engine's decision-making process.

## 1. Recursive Search: Exact Puzzle Solver
...
*   **New Component:** `cpp/DacSolver.hpp`
*   **Algorithm:**
    ...
    *   **Enumerate:** If no rotation fits the current constraints, return `false` and return to the previous tile to try a different rotation.

## 2. Sorting: Heuristic Variable Ordering
**Goal:** Optimize the recursive solver by intelligently choosing *which* tile to solve next, rather than just going in order (0, 1, 2...).

*   **Integration:** Inside `DacSolver`.
*   **Algorithm:**
...
*   **Algorithm:**
    ...
    *   **Usage:** This custom sort function (`sortTiles_dac`) will be called by the solver before starting the recursion.

## 4. Dynamic Programming: Connectivity Memoization
**Goal:** Drastically reduce the computational overhead of validity checks during the recursive enumeration process.

*   **New Component:** `cpp/ConnectivityCache.hpp`
*   **Algorithm:**
    *   **Problem:** In the recursive loop, the engine repeatedly checks "Does Tile T rotated R connect with Neighbor N?". Doing this via raw logic (checking booleans) millions of times is slow.
...
3.  **Solver (Recursive):** The engine starts the recursive enumeration process (`solve_dac`) on the sorted list.
